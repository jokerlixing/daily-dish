(function () {
  ...
  function methodButton(recipe, attributes, primary=false) {
    return `<button type="button" class="button ${primary?'button-primary':'button-light'} recipe-method-button" ${attributes} aria-label="查看${escape(recipe.name)}做法"><span>看做法</span>${icon('arrow')}</button>`;
  }
  ...
  function renderMenu() {
    ...
    $('#menu-grid').innerHTML=state.menu.map((recipe,index)=>`<article class="menu-card"><button class="menu-card-open" data-menu-recipe="${escape(recipe.id)}" aria-pressed="${recipe.id===state.current?.id}" aria-label="选择${escape(recipe.name)}"><span class="menu-photo">${imageMarkup(recipe,true)}<span class="menu-number">${index+1}</span></span><span class="menu-card-content"><strong>${escape(recipe.name)}</strong><span>${escape(recipe.cuisine)} · ${recipe.time} 分钟</span></span></button><div class="recipe-card-actions">${methodButton(recipe,`data-menu-cooking="${escape(recipe.id)}"`)}${addTodayButton(recipe)}</div></article>`).join('');
  }
  ...
  function renderCatalog() {
    ...
    $('#catalog-grid').innerHTML=visible.map(recipe=>`<article class="catalog-card" data-catalog-recipe="${escape(recipe.id)}"><button class="catalog-open" aria-label="打开${escape(recipe.name)}菜谱"><span class="catalog-photo">${imageMarkup(recipe,true)}</span><span class="catalog-card-body"><span class="catalog-card-tags">${escape(recipe.category||recipe.type)} · ${escape(recipe.cuisine)}</span><strong>${escape(recipe.name)}</strong><span class="catalog-card-meta">${recipe.time} 分钟 <span>${escape(recipe.difficulty)}</span></span></span></button><div class="recipe-card-actions">${methodButton(recipe,`data-catalog-cooking="${escape(recipe.id)}"`)}${addTodayButton(recipe)}</div></article>`).join('');
  ...
  function renderCollection() {
    ...
    $('#dialog-content').innerHTML=toolbar+(list.length?list.map(recipe=>`<article class="collection-row"><span class="collection-photo">${imageMarkup(recipe,true)}</span><div class="collection-info"><h3>${escape(recipe.name)}</h3><p>${escape(recipe.cuisine)} · ${recipe.time} 分钟 · ${escape(recipe.flavor)}</p></div><div class="collection-actions">${methodButton(recipe,`data-view="${escape(recipe.id)}"`)}${addTodayButton(recipe)}${favorites?`<button class="remove-favorite" data-remove="${escape(recipe.id)}" aria-label="取消收藏${escape(recipe.name)}">${icon('close')}</button>`:''}</div></article>`).join(''):`<div class="collection-empty">${icon(favorites?'heart':'clock')}<h3>${favorites?'还没有收藏的味道':'还没有抽签记录'}</h3><p>${favorites?'遇到想吃的菜，点“收藏这道菜”，下次就能在这里找到。':'点一下随机按钮，开始今天的美味冒险。'}</p></div>`);
  }
  ...
  function fridgeCard(match) {
    ...
    return `<article class="fridge-card"><div class="fridge-card-photo">${imageMarkup(recipe,true)}</div><div class="fridge-card-body"><span class="match-badge ${canCook?'is-ready':''}">${canCook?'主料已齐':`还差 ${missing.length} 样`}</span><h3>${escape(recipe.name)}</h3><p class="fridge-match">已有：${escape(matched.join('、'))}</p>${missing.length?`<p class="fridge-missing">还缺：${escape(missing.join('、'))}</p>`:'<p class="fridge-missing ready-note">核对用量，就能准备开火</p>'}<p class="fridge-cooking-time">制作约 ${recipe.time} 分钟</p><div class="fridge-card-actions">${methodButton(recipe,`data-fridge-view="${escape(recipe.id)}"`)}${addTodayButton(recipe)}</div></div></article>`;
  }
  ...
  $('#menu-grid').addEventListener('click',event=>{const method=event.target.closest('[data-menu-cooking]'),button=event.target.closest('[data-menu-recipe]');if(method||button){finishRoll();showRecipe(byId.get(method?method.dataset.menuCooking:button.dataset.menuRecipe),false);focusSection(method?'#cooking-steps':'#recipe-name');}});
  ...
  $('#recipe').addEventListener('click',event=>{if(event.target.closest('#favorite-recipe'))toggleFavorite(state.current.id);if(event.target.closest('#start-cooking')){finishRoll();focusSection('#cooking-steps');}});
